...
def place_order(request):
    ...
    for item in cart_items:
        ...
        if extras:
            line += "Додатки:\n"
            for extra in extras:
                line += f"• {extra.name} (+{extra.price} ₴)\n"

        if sauces:
            line += "Соуси:\n"
            for sauce in sauces:
                line += f"• {sauce.name} (+{sauce.price} ₴)\n"

        line += f"Сума: {item_total} ₴"
    ...
    message = (
            ...
            f"📋 Інформація:\n"
            f"👤 Ім'я: {recipient_name}\n"
            f"📍 Адреса: {address}\n"
            f"📱 Телефон: {phone_number}\n\n"
            f"💬 Бажання (comment): {comment if comment else 'No comment'}\n\n"
